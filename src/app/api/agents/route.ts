...
export async function GET() {
  try {
    ...
    // Get agents from config — support both explicit list and implicit single-agent setups
    const agentList: any[] = config.agents?.list || [];
    
    // If no explicit list, synthesize the main agent from defaults
    if (agentList.length === 0) {
      ...
      agentList.push({
        ...
        model: config.agents?.defaults?.model || config.models,
